...
for i in ticker :
    ...
    if comparison_returns[i].max() > 10000 :
        comparison_pricing_plot.add_trace(
            ...
            secondary_y=True)
    else : comparison_pricing_plot.add_trace(go.Scatter(x = comparison_returns.index, y = comparison_returns[i], mode = 'lines', name = i, legendgroup = "group", legendgrouptitle_text = "Left Hand Axis"), secondary_y=False)
